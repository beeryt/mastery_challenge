...
.equ	BTNR = 0				; Right Whisker Input Bit
.equ	BTNL = 2				; Left Whisker Input Bit
.equ	START = 0x04
...
MAIN:
		...
		andi	mpr, BTN_MASK
		cpi		mpr, (1<<BTNL)	; Check for Right Whisker input (Recall Active Low)
		brne	NEXT			; Continue with next check
		rcall	HitRight		; Call the subroutine HitRight
		rjmp	MAIN			; Continue with program
NEXT:	cpi		mpr, (1<<BTNR)	; Check for Left Whisker input (Recall Active)
